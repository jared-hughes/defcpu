...
var require_buffer = __commonJS({
  "../node_modules/buffer/index.js"(exports) {
    ...
    Buffer4.prototype.write = function write(string2, offset, length, encoding) {
      if (offset === void 0) {
        encoding = "utf8";
        length = this.length;
        offset = 0;
...
function header(fields) {
  const result = class {
    ...
    /** @param {T} config */
    constructor(config) {
      for (const name2 of Object.keys(fields))
        this[name2] = config[name2] ?? 0;
      this.#fields = fields;
    }
    dump(wordSize) {
      const data = import_buffer.Buffer.alloc(result.size(wordSize));
      for (const name2 of Object.keys(this.#fields)) {
        let [offset, size] = fields[name2], number3 = this[name2];
        if (Array.isArray(offset)) offset = offset[wordSize >> 6];
        if (Array.isArray(size)) size = size[wordSize >> 6];
        switch (size) {
          case 1:
            data.writeInt8(number3, offset);
            break;
          case 2:
            data.writeInt16LE(number3, offset);
            break;
          case 4:
            data.writeInt32LE(number3, offset);
            break;
          case 8:
            data.writeBigInt64LE(BigInt(number3), offset);
            break;
        }
      }
      return data;
...
function createExecutable(state) {
  const outBuffer = new OutputBuffer();
  let entryPoint = 0, entrySection = state.sections.find((section) => section.name == ".text");
  let programHeaders = [], fileOffset = Math.ceil(ELFHeader.size(state.bitness) / 4096) * 4096, memoryOffset = 4194304;
  let sections2 = [...state.sections];
  let commonSymbols = [];
  state.symbols.forEach((symbol, name2) => {
    if (name2 == "_start" && symbol.bind == 1) {
  ...
  outBuffer.write(new ELFHeader({
    ...
    e_type: 2,
    // ET_EXEC
    e_machine: state.bitness === 64 ? 62 : 3,
    e_version: 1,
    e_entry: entryPoint,
    e_phoff: fileOffset,
  ...
  for (const header2 of programHeaders) {
    outBuffer.write(header2.dump(state.bitness), fileOffset);
...
function getExtensions(onViewUpdate2) {
  ...
  const extensions = [
    ...
    assembly(),
    bracketMatching(),
    EditorView.updateListener.of(onViewUpdate2),
    EditorView.theme({
...
function saveToLocalStorage() {
  const code2 = editor.state.sliceDoc();
  localStorage.setItem("saved-src", code2);
}
var debouncedSave = debounce(saveToLocalStorage, 250);
function onViewUpdate(vu) {
