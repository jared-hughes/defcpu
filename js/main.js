...
function getBreakpointFroms(state2) {
  const breakpoints = state2.field(breakpointField);
  return breakpointFroms(breakpoints);
}
function setBreakpointInit(breakpointFroms2) {
  return breakpointFroms2.map(
    (from) => breakpointEffect.of({ pos: from, on: true })
  );
}
...
function getExtensions(onViewUpdate2, onNewGutters2) {
...
function saveToLocalStorage() {
  ...
  const breakpointFroms2 = getBreakpointFroms(editor.state);
  localStorage.setItem("saved-breakpoints", JSON.stringify(breakpointFroms2));
}
...
function getDefaultBreakpointFroms() {
  ...
  if (!saved) return [];
  try {
    const breakpointFroms2 = JSON.parse(saved);
    if (Array.isArray(breakpointFroms2) && breakpointFroms2.every((x) => typeof x === "number")) {
      return breakpointFroms2;
    }
    return [];
  } catch {
    return [];
